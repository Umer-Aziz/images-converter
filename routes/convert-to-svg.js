const convertToSvg = require('express').Router();
  ...
  const maxSize = 10 * 1024 * 1024 ;
    ...
    var upload = multer({
      ...
      fileFilter: (req, file, cb) => {
        ...
        || file.mimetype == "image/gif" || file.mimetype == "image/avif" || file.mimetype == "image/tiff") {
          cb(null, true);
        } else {
          cb(null, false);
...
convertToSvg.post('/convert-to-svg',(req,res)=>{
    upload(req,res,err =>{
  ...
  let output_path = Date.now() + "-" + "output.svg";
  sharp(req.file.path)
  .toFile(output_path) 
